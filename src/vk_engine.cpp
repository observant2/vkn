...
void VulkanEngine::init() {
  ...
  auto window_flags = (SDL_WindowFlags)(SDL_WINDOW_VULKAN | SDL_WINDOW_SHOWN);
...
void VulkanEngine::draw() {
  ...
  VK_CHECK(vkAcquireNextImageKHR(_device, _swapchain, 1000000000, _presentSemaphore, nullptr,
                                 &swapchainImageIndex));
  ...
  float flash = abs(sin(static_cast<float>(_frameNumber) / 60.f));
...
void VulkanEngine::init_vulkan() {
  ...
  if (SDL_Vulkan_CreateSurface(_window, _instance, &_surface) == SDL_FALSE) {
    fmt::println(stderr, "Surface creation failed: {}", SDL_GetError());
  }
...
void VulkanEngine::init_commands() {
  ...
  auto cmdAllocInfo = vkinit::command_buffer_allocate_info(_commandPool, 1);
